    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-900 flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400">Join thousands of marketers mastering their craft.</p>
        </div>
        ...
        >
          <h2 className="text-2xl font-bold text-center mb-6 text-slate-800 dark:text-slate-200">Create Account</h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {password && (
                <div className="mt-2">
                  <div className="text-xs text-slate-600 dark:text-slate-400 mb-2">Password strength:</div>
                  <div className="space-y-1">
                    <div className={`text-xs flex items-center gap-2 ${passwordValidation.minLength ? 'text-green-600' : 'text-red-500'}`}>
                      <span className={passwordValidation.minLength ? '✓' : '✗'}>•</span>
                      At least 6 characters
                    </div>
                    <div className={`text-xs flex items-center gap-2 ${passwordValidation.hasUpperCase ? 'text-green-600' : 'text-red-500'}`}>
                      <span className={passwordValidation.hasUpperCase ? '✓' : '✗'}>•</span>
                      Uppercase letter
                    </div>
                    <div className={`text-xs flex items-center gap-2 ${passwordValidation.hasLowerCase ? 'text-green-600' : 'text-red-500'}`}>
                      <span className={passwordValidation.hasLowerCase ? '✓' : '✗'}>•</span>
                      Lowercase letter
                    </div>
                    <div className={`text-xs flex items-center gap-2 ${passwordValidation.hasNumbers ? 'text-green-600' : 'text-red-500'}`}>
                      <span className={passwordValidation.hasNumbers ? '✓' : '✗'}>•</span>
                      Number
                    </div>
                  </div>
                </div>
            ...
            <div>
              <label className="flex items-start gap-3">
                <input
                  ...
                  className="rounded text-coral-500 focus:ring-coral-500 mt-0.5"
                />
            ...
            <motion.button
              type="submit"
              disabled={loading || !passwordValidation.isValid || password !== confirmPassword || !agreeToTerms}
        ...
        <div className="text-center mt-6">
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Join the marketing revolution. Start your journey today.
          </p>
        </div>
      </motion.div>
    </div>
